    ...
    pub fn load_graph(&mut self, graph: JsValue) {
        ...
        let nodes: Vec<Rc<RefCell<dyn Source>>> =  graph.nodes.iter().map(|node: &JSNode| {
            let core_node: Rc<RefCell<dyn Source>> = match node {
                JSNode::MultiplyNodeJS(multiply_node_js) => rc_refcell!(MultiplyNode::new(multiply_node_js.metadata.id, multiply_node_js.multiplicand_source_id, multiply_node_js.multiplier_source_id)),
                JSNode::SumNodeJS(sum_node_js) => rc_refcell!(SumNode::new(sum_node_js.metadata.id, sum_node_js.augend_source_id, sum_node_js.addend_source_id)) ,
